...
class TripleAttention(nn.Module):
    ...
    def forward(self,vocal,vision,emb):
        ...
        a_one_vision = F.softmax(self.Wvision_h1(h_one_vision),dim=-2)  ## along dimsions
        ...
        a_two_vision = F.softmax(self.Wvision_h2(h_two_vision),dim=-2)
...
def save_checkpoint(state, is_final, filename='attention_net'):
    ...
    os.system("mkdir -p TAN_1024_denseacrosstime_gated")
    torch.save(state, './TAN_1024_denseacrosstime_gated/'+filename)
    if is_final:
...
while epoch<no_of_epochs:
    ...
    if use_pretrained:
        ...
        pretrained_file = './TAN_1024_denseacrosstime_gated/triple_attention_net__5.pth.tar'

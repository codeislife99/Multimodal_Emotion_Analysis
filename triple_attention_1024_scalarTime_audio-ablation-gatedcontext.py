...
class TripleAttention(nn.Module):
	def __init__(self,no_of_emotions,dan_hidden_size,attention_hidden_size):
		...
		self.Wvocal_gh1 = nn.Linear(N2,1)
		...
		self.Wvocal_gh2 = nn.Linear(N2,1)
	...
	def forward(self,vocal,vision,emb):
		N = dan_hidden_size
		N2 = attention_hidden_size
		one_constant_vocal = Variable(torch.ones(vocal.size()).float()).cuda()
		# Sorting out vision
		...
		# a_one_vocal = F.tanh(self.Wvocal_gh1(h_one_vocal))*F.softmax(self.Wvocal_h1(h_one_vocal),dim=0)
		# vocal_one = (a_one_vocal.repeat(1,N)*vocal).mean(0).unsqueeze(0)
		gate_one_vocal = F.sigmoid(self.Wvocal_gh1(h_one_vocal))
		vocal_one_2d = (gate_one_vocal.repeat(1,N)*vocal) + ((1-gate_one_vocal).repeat(1,N)*one_constant_vocal)
		vocal_one = vocal_one_2d.mean(0).unsqueeze(0)
		...
		gate_two_vocal = F.sigmoid(self.Wvocal_gh2(h_two_vocal))
		vocal_two_2d = ((gate_two_vocal.repeat(1,N))*vocal) + ((1-gate_two_vocal).repeat(1,N)*one_constant_vocal)
		vocal_two = vocal_two_2d.mean(0).unsqueeze(0)
...
def save_checkpoint(state, is_final, filename='attention_net'):
	...
	os.system("mkdir -p TAN_1024_scalarTime_audio-ablation_gatedcxt") 
	torch.save(state, './TAN_1024_scalarTime_audio-ablation_gatedcxt/'+filename)
	if is_final:
...
while epoch<no_of_epochs:
	...
	running_corrects = 0
	if use_pretrained:
		pretrained_file = './TAN_1024_scalarTime_audio-ablation_gatedcxt/triple_attention_net__1.pth.tar'
	...
	vocal_mean = torch.zeros(vocal_input_size)
	vocal_std = torch.zeros(vocal_input_size)
	vocal_all_size = 0
	for i,(vision,vocal,emb,gt) in enumerate(data_loader):
		vocal_mean = vocal_mean + torch.mul((vocal.float().mean(dim=1).squeeze(0)) , (float(vocal.size()[1])/1000))
		vocal_std = vocal_std + torch.mul((vocal.float().std(dim=1).squeeze(0)) , (float(vocal.size()[1])/1000))
		vocal_all_size += float(vocal.size()[1])/1000
		if i%1000==0 and i>0:
			print('------- Computing vocal mean '+str(i)+' --------')
	vocal_mean.div_(vocal_all_size)
	vocal_std.div_(vocal_all_size)
        #####################################################
	...
	for i,(vision,vocal,emb,gt) in enumerate(data_loader):
		if use_CUDA:
			...
			if epoch>=2 and i%2 == 0:	
				vocal_noised = torch.zeros(batch_size,vocal.size()[1],vocal.size()[2])
				for j in range(0,vocal.size()[1]):
					vocal_noised[0,j,:] = torch.normal(vocal_mean,vocal_std).add_(vocal[0,j,:].float())
				vocal = Variable(vocal_noised.float()).cuda()
			else:
				vocal = Variable(vocal.float()).cuda()
	                ##############################################

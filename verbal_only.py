...
params = list(Wordvec_encoder.parameters()) + list(Predictor.parameters())
...
def save_checkpoint(state, is_final, filename='verbal_net'):
	filename = filename +'_'+str(state['epoch'])+'.pth.tar'
	os.system("mkdir -p verbal_only") 
	torch.save(state, './verbal_only/'+filename)
	if is_final:

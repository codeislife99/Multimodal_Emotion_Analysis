...
def save_checkpoint(state, is_final, filename='attention_net'):
	...
	torch.save(state, './DAN_SONG/'+filename)
	if is_final:
...
while epoch<no_of_epochs:
	...
	if use_CREMAD:
		...
		pretrained_file = './DAN_SONG/dual_attention_net__3.pth.tar'
		checkpoint = torch.load(pretrained_file)
		Vocal_encoder.load_state_dict(checkpoint['Vocal_encoder'])
		Vision_encoder.load_state_dict(checkpoint['Vision_encoder'])
		Attention.load_state_dict(checkpoint['Attention'])
	...
	for idx,csv_file_path in enumerate(all_csv_files):
		...
		for iteration in range(2):
			...
			# print(memory_output.size())


			# else:
			# 	resnet_output2 = Variable(torch.from_numpy(target_df2)).cuda().unsqueeze(0)
			# 	resnet_output3 = Variable(torch.from_numpy(target_df3)).cuda().unsqueeze(0)
			# 	target1 = Variable(torch.from_numpy(target_numpy)).cuda()  
			# 	vocal_seq_i2 = Variable(torch.from_numpy(vocal_seq_input2)).cuda()
			# 	vocal_seq_i3 = Variable(torch.from_numpy(vocal_seq_input3)).cuda()
			# 	vocal_output2 = Vocal_encoder(vocal_seq_i2)
			# 	vocal_output3 = Vocal_encoder(vocal_seq_i3)
			# 	vision_output2 = Vision_encoder(resnet_output2)
			# 	vision_output3 = Vision_encoder(resnet_output3)
			# 	outputs2 = Attention(vocal_output2,vision_output2)
			# 	outputs3 = Attention(vocal_output3,vision_output3)
			# 	outputs = torch.cat(outputs0,outputs1)

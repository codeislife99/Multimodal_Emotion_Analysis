...
def save_checkpoint(state, is_final, filename='attention_net'):
	...
	os.system("mkdir -p TAN_1024_scalarTime_pretrainedencoders") 
	torch.save(state, './TAN_1024_scalarTime_pretrainedencoders/'+filename)
	if is_final:
...
while epoch<no_of_epochs:
	...
	if use_pretrained_encoders:
		...
		use_pretrained_encoders = False
	if use_pretrained:
		pretrained_file = './TAN_1024_scalarTime_pretrainedencoders/triple_attention_net__1.pth.tar'

...
from models.highway import GatedMemUpdate_B
...
class TripleAttention(nn.Module):
    def __init__(self,no_of_emotions,dan_hidden_size,attention_hidden_size,gated_mem):
        ...
        self.Wvisionvision_gh = nn.Linear(N2,1)
        self.Wvisionvocal_gh = nn.Linear(N2,1)
        self.Wvisionemb_gh = nn.Linear(N2,1)
        self.Wvocalvision_gh = nn.Linear(N2,1)
        self.Wvocalvocal_gh = nn.Linear(N2,1)
        self.Wvocalemb_gh = nn.Linear(N2,1)
        self.Wembvision_gh = nn.Linear(N2,1)
        self.Wembvocal_gh = nn.Linear(N2,1)
        self.Wembemb_gh = nn.Linear(N2,1)
    ...
    def forward(self,vocal,vision,emb):
        ...
        # Visual Attention
        gate_one_vision = F.sigmoid(self.Wvisionvision_gh(h_one_vision.mean(0).unsqueeze(0)) + self.Wvisionvocal_gh(h_one_vocal.mean(0).unsqueeze(0)) + self.Wvisionemb_gh(h_one_emb.mean(0).unsqueeze(0)))
        ...
        # Vocal Attention
        gate_one_vocal = F.sigmoid(self.Wvocalvision_gh(h_one_vision.mean(0).unsqueeze(0)) + self.Wvocalvocal_gh(h_one_vocal.mean(0).unsqueeze(0)) + self.Wvocalemb_gh(h_one_emb.mean(0).unsqueeze(0)))
        ...
        # Emb Attention
        gate_one_emb = F.sigmoid(self.Wembvision_gh(h_one_vision.mean(0).unsqueeze(0)) + self.Wembvocal_gh(h_one_vocal.mean(0).unsqueeze(0)) + self.Wembemb_gh(h_one_emb.mean(0).unsqueeze(0)))
        ...
        # Memory Update
        if self.gated_mem:
            concated = torch.cat((vision_one, vocal_one, emb_one), 1)
        ...
        # Visual Attention
        gate_two_vision = F.sigmoid(self.Wvisionvision_gh(h_two_vision.mean(0).unsqueeze(0)) + self.Wvisionvocal_gh(h_two_vocal.mean(0).unsqueeze(0)) + self.Wvisionemb_gh(h_two_emb.mean(0).unsqueeze(0)))
        ...
        # Vocal Attention
        gate_two_vocal = F.sigmoid(self.Wvocalvision_gh(h_two_vision.mean(0).unsqueeze(0)) + self.Wvocalvocal_gh(h_two_vocal.mean(0).unsqueeze(0)) + self.Wvocalemb_gh(h_two_emb.mean(0).unsqueeze(0)))
        ...
        # Emb Attention
        gate_two_emb = F.sigmoid(self.Wembvision_gh(h_two_vision.mean(0).unsqueeze(0)) + self.Wembvocal_gh(h_two_vocal.mean(0).unsqueeze(0)) + self.Wembemb_gh(h_two_emb.mean(0).unsqueeze(0)))
        ...
        # Memory Update
        if self.gated_mem:
            concated = torch.cat((vision_two, vocal_two, emb_two), 1)
        ...
        # Visual Attention
        gate_three_vision = F.sigmoid(self.Wvisionvision_gh(h_three_vision.mean(0).unsqueeze(0)) + self.Wvisionvocal_gh(h_three_vocal.mean(0).unsqueeze(0)) + self.Wvisionemb_gh(h_three_emb.mean(0).unsqueeze(0)))
        ...
        # Vocal Attention
        gate_three_vocal = F.sigmoid(self.Wvocalvision_gh(h_three_vision.mean(0).unsqueeze(0)) + self.Wvocalvocal_gh(h_three_vocal.mean(0).unsqueeze(0)) + self.Wvocalemb_gh(h_three_emb.mean(0).unsqueeze(0)))
        ...
        # Emb Attention
        gate_three_emb = F.sigmoid(self.Wembvision_gh(h_three_vision.mean(0).unsqueeze(0)) + self.Wembvocal_gh(h_three_vocal.mean(0).unsqueeze(0)) + self.Wembemb_gh(h_three_emb.mean(0).unsqueeze(0)))
        ...
        # Memory Update
        if self.gated_mem:
            concated = torch.cat((vision_three, vocal_three, emb_three), 1)
...
def save_checkpoint(state, is_final, filename='attention_net'):
    ...
    os.system("mkdir -p TAN_both_gates")
    torch.save(state, './TAN_both_gates/'+filename)
    if is_final:
...
while epoch<no_of_epochs:
    ...
    running_corrects = 0
    if use_pretrained:
        pretrained_file = './TAN_both_gates/triple_attention_net__4.pth.tar'

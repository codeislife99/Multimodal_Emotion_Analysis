...
class MultimodalDataset(object):
    ...
    def __init__(self, dataset, visual='facet', audio='covarep', text='embeddings', pivot='words', sentiments=False, emotions=True, max_len=20):
        ...
        if sentiments:
            self.train_set_labels = np.array([self.sentiments[vid][sid] for (vid, train_set_ids) in train_set_ids])
        ...
        if emotions:
            self.train_set_labels = np.array([self.emotions[vid][sid] for (vid, train_set_ids) in train_set_ids])
            self.valid_set_labels = np.array([self.emotions[vid][sid] for (vid, sid) in valid_set_ids])
            self.test_set_labels = np.array([self.emotions[vid][sid] for (vid, sid) in test_set_ids])

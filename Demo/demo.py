...
class DualAttention(nn.Module):
    def __init__(self,no_of_emotions):
        ...
        ''' K= 1 ''' 
        self.Wvision_1 = nn.Linear(512,512)
        self.Wvision_m1 = nn.Linear(512,512)
        self.Wvision_h1 = nn.Linear(512,512)
        self.Wvocal_1 = nn.Linear(512,512)
        self.Wvocal_m1 = nn.Linear(512,512)
        self.Wvocal_h1 = nn.Linear(512,512)

        ''' K = 2 '''
        self.Wvision_2 = nn.Linear(512,512)
        self.Wvision_m2 = nn.Linear(512,512)
        self.Wvision_h2 = nn.Linear(512,512)
        self.Wvocal_2 = nn.Linear(512,512)
        self.Wvocal_m2 = nn.Linear(512,512)
        self.Wvocal_h2 = nn.Linear(512,512)

        self.fc = nn.Linear(512, no_of_emotions)


    def forward(self,vocal,resnet_output):
        # Sorting out vision
        ...
        a_one_vision = F.softmax(self.Wvision_h1(h_one_vision))
        ...
        a_one_vocal = F.softmax(self.Wvocal_h1(h_one_vocal))
        ...
        a_two_vision = F.softmax(self.Wvision_h2(h_two_vision))
        # # print(a_two_vision.size())
        # hm = []
        # print(a_two_vision)
        # print(torch.max(a_two_vision,1))
        # for i in range(49):
        #   print(i)
        #   hm.append(a_two_vision.max(1)[i].data.cpu().numpy()[0])
        # resized_arr = cv2.resize(np.reshape(np.array(hm),(7,7)), (7, 7), interpolation = cv2.INTER_CUBIC)
        # plt.imshow(resized_arr, cmap=cm.gray, interpolation='nearest')
        # plt.show()
        ...
        a_two_vocal = F.softmax(self.Wvocal_h2(h_two_vocal))

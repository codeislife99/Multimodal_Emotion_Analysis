...
class TripleAttention(nn.Module):
	def __init__(self,no_of_emotions,dan_hidden_size,attention_hidden_size):
		...
		self.Wvision_gh1 = nn.Linear(N2,1)
		...
		self.Wvision_gh2 = nn.Linear(N2,1)
	...
	def forward(self,vocal,vision,emb):
		N = dan_hidden_size
		N2 = attention_hidden_size
		one_constant_vision = Variable(torch.ones(vision.size()).float()).cuda()
		# Sorting out vision
		...
		gate_one_vision = F.sigmoid(self.Wvision_gh1(h_one_vision))
		vision_one_2d = (gate_one_vision.repeat(1,N)*vision) + ((1-gate_one_vision).repeat(1,N)*one_constant_vision)
		vision_one = vision_one_2d.mean(0).unsqueeze(0)
		...
		gate_two_vision = F.sigmoid(self.Wvision_gh2(h_two_vision))
		vision_two_2d = ((gate_two_vision.repeat(1,N))*vision) + ((1-gate_two_vision).repeat(1,N)*one_constant_vision)
		vision_two = vision_two_2d.mean(0).unsqueeze(0)
...
def save_checkpoint(state, is_final, filename='attention_net'):
	...
	os.system("mkdir -p TAN_1024_scalarTime_vision-ablation_gatedcxt") 
	torch.save(state, './TAN_1024_scalarTime_vision-ablation_gatedcxt/'+filename)
	if is_final:
...
while epoch<no_of_epochs:
	...
	running_corrects = 0
	if use_pretrained:
		pretrained_file = './TAN_1024_scalarTime_vision-ablation_gatedcxt/triple_attention_net__1.pth.tar'

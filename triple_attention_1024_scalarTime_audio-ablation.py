...
def save_checkpoint(state, is_final, filename='attention_net'):
	...
	os.system("mkdir -p TAN_1024_scalarTime_audio-ablation") 
	torch.save(state, './TAN_1024_scalarTime_audio-ablation/'+filename)
	if is_final:
...
while epoch<no_of_epochs:
	...
	running_corrects = 0
	if use_pretrained:
		pretrained_file = './TAN_1024_scalarTime_audio-ablation/triple_attention_net__4.pth.tar'

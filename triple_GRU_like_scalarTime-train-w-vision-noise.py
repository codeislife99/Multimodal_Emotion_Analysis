...
def save_checkpoint(state, is_final, filename='attention_net'):
    ...
    os.system("mkdir -p TAN_GRU_like_scalarTime-train-w-vision-noise")
    torch.save(state, './TAN_GRU_like_scalarTime-train-w-vision-noise/'+filename)
    if is_final:
...
while epoch<no_of_epochs:
    ...
    running_corrects = 0
    if use_pretrained:
        pretrained_file = './TAN_GRU_like_scalarTime-train-w-vision-noise/triple_attention_net__4.pth.tar'

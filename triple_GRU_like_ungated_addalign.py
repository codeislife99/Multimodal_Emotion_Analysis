...
class TripleAttention(nn.Module):
    ...
    def forward(self,vocal,vision,emb):
        ...
        '--------------------------------------------------K = 1 ---------------------------------------------------'
        # Visual Attention
        h_one_vision = F.tanh(self.Wvision_1(vision) + self.Wvision_m1(m_zero_vision)) # *F.tanh(self.Wvision_m1(m_zero_vision))
        ...
        # Vocal Attention
        h_one_vocal = F.tanh(self.Wvocal_1(vocal) + self.Wvocal_m1(m_zero_vocal)) # *F.tanh(self.Wvocal_m1(m_zero_vocal))
        ...
        # Emb Attention
        h_one_emb = F.tanh(self.Wemb_1(emb) + self.Wemb_m1(m_zero_emb)) # *F.tanh(self.Wemb_m1(m_zero_emb))
        ...
        # Visual Attention
        h_two_vision = F.tanh(self.Wvision_1(vision) + self.Wvision_m1(m_one_vision))
        a_two_vision = F.softmax(self.Wvision_h1(h_two_vision),dim=0)
        ...
        # Vocal Attention
        h_two_vocal = F.tanh(self.Wvocal_1(vocal) + self.Wvocal_m1(m_one_vocal))
        a_two_vocal = F.softmax(self.Wvocal_h1(h_two_vocal),dim=0)
        ...
        # Emb Attention
        h_two_emb = F.tanh(self.Wemb_1(emb) + self.Wemb_m1(m_one_emb))
        a_two_emb = F.softmax(self.Wemb_h1(h_two_emb),dim=0)
        ...
        # Visual Attention
        h_three_vision = F.tanh(self.Wvision_1(vision) + self.Wvision_m1(m_two_vision))
        a_three_vision = F.softmax(self.Wvision_h1(h_three_vision),dim=0)
        ...
        # Vocal Attention
        h_three_vocal = F.tanh(self.Wvocal_1(vocal) + self.Wvocal_m1(m_two_vocal))
        a_three_vocal = F.softmax(self.Wvocal_h1(h_three_vocal),dim=0)
        ...
        # Emb Attention
        h_three_emb = F.tanh(self.Wemb_1(emb) + self.Wemb_m1(m_two_emb))
        a_three_emb = F.softmax(self.Wemb_h1(h_three_emb),dim=0)
...
def save_checkpoint(state, is_final, filename='attention_net'):
    ...
    os.system("mkdir -p TAN_GRU_ungated_baseline")
    torch.save(state, './TAN_GRU_ungated_baseline/'+filename)
    if is_final:
...
while epoch<no_of_epochs:
    ...
    running_corrects = 0
    if use_pretrained:
        pretrained_file = './TAN_GRU_ungated_baseline/triple_attention_net__4.pth.tar'

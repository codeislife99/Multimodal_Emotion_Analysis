...
class VocalNet(nn.Module):
    ...
    def forward(self, vocal_input):
        x = F.leaky_relu(F.max_pool1d(self.bn1(self.conv1(vocal_input)), 2))
        x = F.leaky_relu(F.max_pool1d(self.bn2(self.conv2(x)), 5))
        x = x.view(-1, vocalnet_output_size) # Check this out. View causing problems
        return x
...
no_of_epochs = 1000
show_image = True
writer = SummaryWriter('./logs_CREMAD_test')
...
target_seq_total = ""
if test_mode:
    directory = "./all_sequences/val_30"
else:
...
if not read_prev_list:
    ...
    if not test_mode:
        ...
        with open("input_list_train.pickle", 'wb') as g:
            pickle.dump(input_list, g)
    else:
        print('Size of Test/Val Set = ' + str(len(input_list)))
        # input_list = input_list[:4]
        # with open("input_list_val.pickle", 'wb') as g:
        #     pickle.dump(input_list, g)        
else:
...
for epoch in range(curr_epoch, no_of_epochs):  
    ...
    running_corrects = 0
    if use_pretrained:
        checkpoint = torch.load('resnet18_vocalnet_LSTM_110100_0.pth.tar')
    ...
    for j in range(j_start, len(input_list), batch_size):
        ...
        if ((len(input_list) - j) > batch_size): # I changed this from len(sequences) to len(input_list)

...
def save_checkpoint(state, is_final, filename='attention_net'):
    ...
    os.system("mkdir -p TAN_1024_scalarTime_pretrained_gated_k3") 
    torch.save(state, './TAN_1024_scalarTime_pretrained_gated_k3/'+filename)
    if is_final:
...
while epoch<no_of_epochs:
    ...
    if use_pretrained:
        pretrained_file = './TAN_1024_scalarTime_pretrained_gated_k3/triple_attention_net__3.pth.tar'
